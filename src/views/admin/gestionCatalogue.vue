      ...
      <div class="card-body py-4">
        <div class="table-responsive">
          <table class="table table-row-dashed table-row-gray-300 gy-7">
            ...
            <tbody
              v-loading="catalogueStore.catalogueLoader"
              element-loading-background="#151521"
            >
              <tr
                v-for="(catalogue, index) in catalogueStore.catalogues"
                :key="index"
              >
                <td>{{ catalogue.id }}</td>
                ...
                <td>
                  <img
                    class="couverture"
                    src="https://www.purplebird.fr/img/image1.8e39dac6.jpg"
                    v-if="catalogue.url"
                    alt="couverture"
                  />
                </td>
        ...
        <div class="row">
          ...
          >
            <table-pagination
              :currentPage="page"
              :perPage="perPage"
              :totalPages="catalogueStore.catalogueTotalPages"
              :total="catalogueStore.catalogueTotal"
      ...
      <div class="modal-dialog modal-dialog-scrollable">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" v-if="!current_edit">Nouveau catalogue</h5>
            <h5 class="modal-title" v-else>Modifier catalogue</h5>
          ...
          <div class="modal-body scroll-y">
            ...
            >
              <div class="d-flex flex-column mb-7 fv-row">
                ...
                <input
                  v-model="new_catalogue.category"
                  type="text"
                  name="category"
...
export default {
  ...
  data() {
    return {
      ...
      search: "",
      catalogueStore: useCatalogueStore(),
  ...
  methods: {
    ...
    setup(reset = false) {
      ...
      this.catalogueStore.all_catalogue({
    ...
    editItem(item) {
      ...
      this.new_catalogue = { ...item };
    },
    deleteItem(item) {
      Swal.fire({
        background: "#1e1e2d",
        text: "Êtes-vous sûr de vouloir supprimer " + item.id + " ?",
        icon: "warning",
        ...
        if (response.value) {
          this.catalogueStore.delete_catalogue(item).then((response) => {
            if (response) {
              this.setup();
              ElNotification({
                title: "Succès",
                message: "Le Catalogue a été supprimé.",
    ...
    handleStore() {
      ...
      if (!this.current_edit) {
        this.catalogueStore
          .store_catalogue(this.new_catalogue)
          .then((valid) => {
        ...
        this.catalogueStore.edit_catalogue(this.new_catalogue).then((valid) => {
          submitButtonRef.disabled = false;
          submitButtonRef?.removeAttribute("data-kt-indicator");
          if (valid) {
            this.setup();
