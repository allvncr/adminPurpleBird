      ...
      <div class="card-body py-4">
        <div class="table-responsive">
          <table class="table table-row-dashed table-row-gray-300 gy-7">
            ...
            <tbody
              v-loading="roleStore.roleLoader"
              element-loading-background="#151521"
            >
              <tr v-for="(role, index) in roleStore.roles" :key="index">
                <td>{{ role.id }}</td>
                <td>{{ role.name }}</td>
                <td>
        ...
        <div class="row">
          ...
          >
            <table-pagination
              :currentPage="page"
              :perPage="perPage"
              :totalPages="roleStore.roleTotalPages"
              :total="roleStore.roleTotal"
      ...
      <div class="modal-dialog modal-dialog-scrollable">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" v-if="!current_edit">Nouveau role</h5>
            <h5 class="modal-title" v-else>Modifier role</h5>
          ...
          <div class="modal-body scroll-y">
            ...
            >
              ...
              <div class="d-flex flex-column mb-7 fv-row">
                ...
                <el-select
                  multiple
                  placeholder="Selectionner permissions"
                  class="form-control form-control-solid"
...
export default {
  ...
  methods: {
    ...
    editItem(item) {
      ...
      this.new_role = { ...item };
      this.new_role.permissions = this.new_role.permissions.map((el) => el.id);
    },
    deleteItem(item) {
      Swal.fire({
        background: "#1e1e2d",
        ...
        if (response.value) {
          this.roleStore.delete_role(item).then((response) => {
            if (response) {
              this.setup();
              ElNotification({
                title: "Succès",
    ...
    handleStore() {
      ...
      if (!this.current_edit) {
        this.roleStore.store_role(this.new_role).then((valid) => {
        ...
        this.roleStore.edit_role(this.new_role).then((valid) => {
          submitButtonRef.disabled = false;
          submitButtonRef?.removeAttribute("data-kt-indicator");
          if (valid) {
            this.setup();
  ...
  mounted() {
    ...
    this.permissionStore.all_permission({
      page: 1,
      per_page: 1000,
    });
