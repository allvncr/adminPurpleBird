      ...
      >
        ...
        >
          ...
          >
            ...
            <!--begin::Item-->
            <li class="breadcrumb-item text-muted">Categorie Photo</li>
            <!--end::Item-->
          </ul>
          <!--end::Breadcrumb-->
        </div>
      ...
      <div class="card-body py-4">
        <div class="table-responsive">
          <table class="table table-row-dashed table-row-gray-300 gy-7">
            ...
            <tbody
              v-loading="categorieStore.categorieLoader"
              element-loading-background="#151521"
            >
              <tr
                v-for="(category, index) in categorieStore.categories"
                :key="index"
              >
                <td>{{ category.id }}</td>
        ...
        <div class="row">
          ...
          >
            <table-pagination
              :currentPage="page"
              :perPage="perPage"
              :totalPages="categorieStore.categorieTotalPages"
              :total="categorieStore.categorieTotal"
      ...
      <div class="modal-dialog modal-dialog-scrollable">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" v-if="!current_edit">Nouvelle categorie</h5>
...
<script>
import { useCategoriePhotoStore } from "../../stores/categoriePhoto";
...
export default {
  ...
  data() {
    return {
      ...
      search: "",
      categorieStore: useCategoriePhotoStore(),
  ...
  methods: {
    ...
    deleteItem(item) {
      Swal.fire({
        ...
        if (response.value) {
          this.categorieStore.delete_categorie(item).then((response) => {
            if (response) {
              this.setup();
              ElNotification({
                title: "Succès",
    ...
    handleStore() {
      ...
      if (!this.current_edit) {
        this.categorieStore
          .store_categorie(this.new_categorie)
          .then((valid) => {
        ...
        this.categorieStore.edit_categorie(this.new_categorie).then((valid) => {
          submitButtonRef.disabled = false;
          submitButtonRef?.removeAttribute("data-kt-indicator");
          if (valid) {
            this.setup();
            ElNotification({
              title: "Succès",
              message: "Categorie modifié avec succès",
              position: "bottom-left",
              type: "success",
              customClass: "alert-success",
            });
  ...
  mounted() {
    ...
    var myModalEl = this.$refs.newCardModalRef;
