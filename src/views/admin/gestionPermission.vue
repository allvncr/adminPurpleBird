      ...
      <div class="card-body py-4">
        <div class="table-responsive">
          <table class="table table-row-dashed table-row-gray-300 gy-7">
            ...
            <tbody
              v-loading="permissionStore.permissionLoader"
              element-loading-background="#151521"
            >
              <tr
                v-for="(permission, index) in permissionStore.permissions"
                :key="index"
              >
                <td>{{ permission.id }}</td>
        ...
        <div class="row">
          ...
          >
            <table-pagination
              :currentPage="page"
              :perPage="perPage"
              :totalPages="permissionStore.permissionTotalPages"
      ...
      <div class="modal-dialog modal-dialog-scrollable">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h5 class="modal-title" v-else>Modifier permission</h5>
  ...
  methods: {
    ...
    deleteItem(item) {
      Swal.fire({
        ...
        if (response.value) {
          this.permissionStore.delete_permission(item).then((response) => {
            if (response) {
              this.setup();
              ElNotification({
                title: "Succès",
    ...
    handleStore() {
      ...
      if (!this.current_edit) {
        this.permissionStore
          .store_permission(this.new_permission)
          .then((valid) => {
            ...
            if (valid) {
              ...
              ElNotification({
                title: "Succès",
                message: "Permission ajouté avec succès",
                position: "bottom-left",
                type: "success",
                customClass: "alert-success",
              });
        ...
        this.permissionStore
          .edit_permission(this.new_permission)
          .then((valid) => {
            submitButtonRef.disabled = false;
            submitButtonRef?.removeAttribute("data-kt-indicator");
            if (valid) {
