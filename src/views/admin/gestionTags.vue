      ...
      <div class="card-body py-4">
        <div class="table-responsive">
          <table class="table table-row-dashed table-row-gray-300 gy-7">
            ...
            <tbody
              v-loading="tagStore.tagLoader"
              element-loading-background="#151521"
            >
              <tr v-for="(tag, index) in tagStore.tags" :key="index">
                <td>{{ tag.id }}</td>
                <td>{{ tag.title }}</td>
                <td>
        ...
        <div class="row">
          ...
          >
            <table-pagination
              :currentPage="page"
              :perPage="perPage"
              :totalPages="tagStore.tagTotalPages"
              :total="tagStore.tagTotal"
      ...
      <div class="modal-dialog modal-dialog-scrollable">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" v-if="!current_edit">Nouvelle tag</h5>
            <h5 class="modal-title" v-else>Modifier tag</h5>
  ...
  methods: {
    ...
    deleteItem(item) {
      Swal.fire({
        ...
        if (response.value) {
          this.tagStore.delete_tag(item).then((response) => {
            if (response) {
              this.setup();
              ElNotification({
                title: "Succès",
                message: "La tag a été supprimée.",
    ...
    handleStore() {
      ...
      if (!this.current_edit) {
        this.tagStore.store_tag(this.new_tag).then((valid) => {
        ...
        this.tagStore.edit_tag(this.new_tag).then((valid) => {
          submitButtonRef.disabled = false;
          submitButtonRef?.removeAttribute("data-kt-indicator");
          if (valid) {
            this.setup();
