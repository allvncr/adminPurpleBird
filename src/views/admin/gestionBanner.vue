      ...
      <div class="card-body py-4">
        <div class="table-responsive">
          <table class="table table-row-dashed table-row-gray-300 gy-7">
            ...
            <tbody
              v-loading="bannerStore.bannerLoader"
              element-loading-background="#151521"
            >
              <tr v-for="(banner, index) in bannerStore.banners" :key="index">
                <td>{{ banner.id }}</td>
                <td>
                  <img
                    class="couverture"
        ...
        <div class="row">
          ...
          >
            <table-pagination
              :currentPage="page"
              :perPage="perPage"
              :totalPages="bannerStore.bannerTotalPages"
              :total="bannerStore.bannerTotal"
      ...
      <div class="modal-dialog modal-dialog-scrollable">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" v-if="!current_edit">Nouveau banner</h5>
            <h5 class="modal-title" v-else>Modifier banner</h5>
...
export default {
  ...
  methods: {
    ...
    onFileChange(event) {
      ...
      this.new_banner.file = file;
    },

    setup(reset = false) {
      if (reset) this.page = 1;
      this.bannerStore.all_banner({
    ...
    deleteItem(item) {
      Swal.fire({
        ...
        if (response.value) {
          this.bannerStore.delete_banner(item).then((response) => {
            if (response) {
              this.setup();
              ElNotification({
                title: "Succès",
                message: "Le Banner a été supprimé.",
    ...
    handleStore() {
      ...
      if (!this.current_edit) {
        ...
        formData.append("file", this.new_banner.file);

        this.bannerStore.store_banner(formData).then((valid) => {
        ...
        this.bannerStore.edit_banner(this.new_banner).then((valid) => {
          submitButtonRef.disabled = false;
          submitButtonRef?.removeAttribute("data-kt-indicator");
          if (valid) {
            this.setup();
            ...
            ElNotification({
              title: "Succès",
              message: "Banner modifié avec succès",
              position: "bottom-left",
              type: "success",
              customClass: "alert-success",
            });
